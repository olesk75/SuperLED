...
IP = "10.0.0.54"    # Must match Arduino IP address TODO: Make Arduino announce its IP adress somewhere we can look it up
...
if __name__ == "__main__":  # Making sure we don't have problems if importing from this file as a module
	...
	buffer_to_screen.updates = 0                         # We need to set this variable AFTER the funtion definition
	...
	signal.signal(signal.SIGINT, signal_handler)    # Setting up th signal handler to arrange tidy exit if maually interrupted

	#ext_effect(glob.Arduino,'hw_test')             # Test LED display using Arduino funtion
	...
	#################################################################################################################################################
	#   This is the main loop - it processes sensor inputs and timers and controls what goes on the displat and when                                #
